...
public abstract class BinderWindowPopup<T extends Control> implements Binder {
    ...
    /**
     * The listener will be triggered, after the popup is detached and is not under the control of the binder anymore.
     * @param listener
     */
    ...
    /**
     * Tells whether the given coordinates occur within the bounds of the given control
     *
     * @param control
     *            - the control
     * @param displayCursX
     ...
     * @return true if true
     */
    private boolean isCursorInControl(Control control, int displayCursX, int displayCursY) {
        Rectangle rect = control.getBounds();
        Point absPos = getDisplayPos(control);
        Rectangle absRect = new Rectangle(absPos.x, absPos.y, rect.width, rect.height);
        return absRect.contains(displayCursX, displayCursY);
    }
    ...
    private boolean isCursorInControlOrPopup(Control currentControl, Shell currentPopup) {
        ...
        if (currentControl != null) {
            isInControl = isCursorInControl(currentControl, cursorLocation.x, cursorLocation.y);
        }

        if (currentPopup != null) {
            // already display coordinates
            isInPop = isCursorInControl(currentPopup, cursorLocation.x, cursorLocation.y);
        }
    ...
    private T getEventControl(TypedEvent e) {
        ...
        if (!(e.widget.getClass().isAssignableFrom(controlType))) {
